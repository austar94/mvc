...
class MonoLog{
	...
	public function __construct($channel = 'DEV', $dirPath = '/_log') {
		...
		if($Member->get_SEQ_userCode()){
			$this->userCode		=	$Member->get_SEQ_userCode();
		}
	...
	public function log_info($msg, $values = ''){
		if($values){
			if(is_array($values)){
				$this->log->info("[" . $_SERVER['REMOTE_ADDR'] . "][".($this->userCode ? $this->userCode : "비회원")."] - " . $msg, $values);
			} else {
				$this->log->info("[" . $_SERVER['REMOTE_ADDR'] . "][".($this->userCode ? $this->userCode : "비회원")."] - " . $msg . ', values : ' . $values);
			}
		} else {
			$this->log->info("[" . $_SERVER['REMOTE_ADDR'] . "][".($this->userCode ? $this->userCode : "비회원")."] - " . $msg);
		}
	...
	public function log_error($msg, $values = ''){
		if($values){
			if(is_array($values)){
				$this->log->error("[" . $_SERVER['REMOTE_ADDR'] . "][".($this->userCode ? $this->userCode : "비회원")."] - " . $msg, $values);
			} else {
				$this->log->error("[" . $_SERVER['REMOTE_ADDR'] . "][".($this->userCode ? $this->userCode : "비회원")."] - " . $msg . ', values : ' . $values);
			}
		} else {
			$this->log->error("[" . $_SERVER['REMOTE_ADDR'] . "][".($this->userCode ? $this->userCode : "비회원")."] - " . $msg);
		}
	}
	...
	public function log_exceptionErr(\Throwable$e){
		if(SYS_DEBUG){
			$this->log->error('============================================================');
			$this->log->error('error ip : ' . $_SERVER['REMOTE_ADDR']);
			$this->log->error('error request : ' . $_SERVER['REQUEST_URI']);
