...
class FileManager {
	//	디렉토리 생성
	public function setMakeDir($defaultPath, $basePath, $optionPath1='', $optionPath2='', $dirPermission= 0777) {
		//$basicUploadPath		=	$this->fm->setMakeDir($common['defaultPath'], 'boardFile', $data['bID'], '', $common['dirPermission']);
		$uploadPath				=	$defaultPath . '/';

		if (!is_dir($uploadPath)) { @mkdir($uploadPath, (int)$dirPermission);}
	...
	public function uploadFile($source_file, $target_file) {
		...
		// 	$msg->set_msg('사용 가능한 용량이 부족합니다. 관리자에게 문의하세요.');
		// 	return $msg;
		// }

		if ( !$source_file || !$target_file ) {
			$msg->set_msg('잘못된 매개변수입니다. 관리자에게 문의하세요.');
			return $msg;
		}

		if ( file_exists($target_file) ) {
			$msg->set_msg('이미 해당 경로에 존재하는 파일 이름입니다.');
			return $msg;
		}
		...
		if ( file_exists($source_file) ) {
			if ( move_uploaded_file($source_file, $target_file) ) {
				$msg->set_result(true);
				$msg->set_msg("파일이 업로드되었습니다.");
				return $msg;
			} else {
				$msg->set_msg("파일이 업로드에 실패하였습니다.");
				return $msg;
			}
		}
	}
	...
	public function uploadImg($source_file, $target_file, $thumb_file='', $thumb_width=150, $thumb_height=100) {
		...
		if ( file_exists($source_file) && list($source_width, $source_height, $source_type, $source_attr) = getimagesize($source_file) ) {
			if ( $source_width < 1 || $source_height < 1 ) {
				$msg->set_msg('올바르지 못한 이미지 파일입니다.');
				return $msg;
			}

			if ( $source_type > 3 ) {
				$msg->set_msg('이미지 파일은 gif, jpg, png만 가능합니다.');
				return $msg;
			}
	...
	public function createThumbnailImg($source_file, $target_file, $width, $height) {
		...
		if ( file_exists($source_file) && list($source_width, $source_height, $source_type, $source_attr) = getimagesize($source_file) ) {
			...
			if ( function_exists('imagecreatetruecolor') ) {
				...
				$msg->set_msg('썸네일 생성 중 오류가 발생했습니다.');
				return $msg;
			}
			...
			$msg->set_result(true);
			$msg->set_msg("$target_file 썸네일이 생성되었습니다.");
			return $msg;
		}
	}
